(ns glide.main
  "Kotodama WASM entrypoint for the Glide clean-room actor (L4) — Clojure port.
  ...
  facts (`glide.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"glide.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "glide-compat" :tier tier :entities entities} 200])
